...
function getCommandCategories(client) {
  ...
  if (allCatsSet.size === 0) {
    console.warn(
      "getCommandCategories: Could not find categories dynamically, using hardcoded defaults."
    );
      ...
      group
        ...
        .addSubcommand((sub) =>
          sub
            .setName("setup")
            .setDescription(
              "Set up verification channel, role, and custom questions."
            )
            ...
            .addStringOption((opt) =>
              opt
                .setName("question_1")
                .setDescription("Custom question #1 (leave blank to disable).")
                .setRequired(false)
                .setMaxLength(200)
            )
            .addStringOption((opt) =>
              opt
                .setName("question_2")
                .setDescription("Custom question #2 (leave blank to disable).")
                .setRequired(false)
                .setMaxLength(200)
            )
            .addStringOption((opt) =>
              opt
                .setName("question_3")
                .setDescription("Custom question #3 (leave blank to disable).")
                .setRequired(false)
                .setMaxLength(200)
            )
            .addStringOption((opt) =>
              opt
                .setName("question_4")
                .setDescription("Custom question #4 (leave blank to disable).")
                .setRequired(false)
                .setMaxLength(200)
            )
        )
          ...
          sub
            ...
            .addRoleOption((opt) =>
              opt
                .setName("role")
                .setDescription(
                  "The role required (leave blank to remove requirement)."
                )
                .setRequired(false)
            )
        )
  ...
  async execute(interaction, client) {
    ...
    try {
      if (group === "verification" && subcommand === "view") {
        ...
        if (config) {
          const modChannel = config.moderator_channel_id
            ? await guild.channels
                .fetch(config.moderator_channel_id)
                .catch(() => null)
            : null;
          const verRole = config.verified_role_id
          ...
          if (qAdded === 0)
            embed.addFields({
              ...
              value: "No custom questions configured.",
            });
        } else {
        ...
        const generateNonNsfwEmbed = async () => {
          ...
          if (verificationConfig) {
            const modCh = verificationConfig.moderator_channel_id
              ? await guild.channels
                  .fetch(verificationConfig.moderator_channel_id)
                  .catch(() => null)
              : null;
            const verRole = verificationConfig.verified_role_id
              ? await guild.roles
                  .fetch(verificationConfig.verified_role_id)
                  .catch(() => null)
              : null;
            verDesc = `**${
              verificationConfig.enabled ? "✅ Enabled" : "❌ Disabled"
            }** (${modCh || "`Invalid Ch.`"} -> ${
          ...
          let mewbotDesc = mewbotConfig
            ? `**${
                mewbotConfig.enabled ? "✅ Enabled" : "❌ Disabled"
              }** (Mode: ${mewbotConfig.watch_mode || "N/A"})`
            : "`Not Configured`";
          embed.addFields({
            name: "🤖 Mewbot Helper",
            value: mewbotDesc,
          ...
          if (categoriesInfo.nonNsfw.length > 0) {
            ...
            embed.addFields({
              name: "Status",
              value: "No non-NSFW command categories found.",
            });
          }
          setDefaultFooter(embed, clientUser);
          return embed;
        ...
        const generateNsfwEmbed = async () => {
          ...
          embed.addFields({
            name: "\u200B",
            value: "**Command Category Controls (NSFW)**",
          });
          if (categoriesInfo.nsfw.length > 0) {
            ...
            embed.addFields({
              name: "Status",
              value: "No NSFW command categories found.",
            });
          }
          setDefaultFooter(embed, clientUser);
          return embed;
        ...
        collector.on("end", async (collected, reason) => {
          try {
            ...
            const footerText = `Settings view timed out. Last view: ${currentPage.replace(
              "_",
              " "
            )} | ${DEFAULT_BOT_FOOTER_TEXT}`;
            setCustomFooter(
            ...
            if (error.code !== 10008 && error.code !== 10062) {
              console.error(
                "Error removing buttons from timed out settings view:",
                error
              );
            }
          }
      ...
      try {
        if (interaction.replied || interaction.deferred) {
          await interaction
            .editReply({
              content:
                "❌ An unexpected error occurred while running this settings command.",
              embeds: [],
              components: [],
            })
            .catch(console.error);
        ...
        console.error(
          "Failed to send final error reply for /serversettings:",
          e
        );
      }
    }
